...
function tokenNotice(resolution: Exclude<MapboxTokenResolution, { status: "ok" }>): string {
  if (resolution.status === "missing") {
    return "3D map preview is not configured yet. Add a restricted public Mapbox token to enable it.";
  }
  if (resolution.reason === "secret_token") {
    return "The 3D map rejected a secret token. Use a restricted public Mapbox token instead.";
  }
  return "The public Mapbox token could not be validated. The 2D route view remains available.";
}
